...
export namespace token {
  ...
  export class get_vault_count_arguments {
    static encode(message: get_vault_count_arguments, writer: Writer): void {
    ...
    static decode(reader: Reader, length: i32): get_vault_count_arguments {
      const end: usize = length < 0 ? reader.end : reader.ptr + length;
      const message = new get_vault_count_arguments();
  ...
  @unmanaged
  export class get_vault_count_result {
    static encode(message: get_vault_count_result, writer: Writer): void {
      if (message.value != 0) {
        writer.uint32(8);
        writer.uint64(message.value);
      }
    ...
    static decode(reader: Reader, length: i32): get_vault_count_result {
      const end: usize = length < 0 ? reader.end : reader.ptr + length;
      const message = new get_vault_count_result();
  ...
  export class drop_rewards_arguments {
    static encode(message: drop_rewards_arguments, writer: Writer): void {
      ...
      if (unique_name_to.length !== 0) {
        for (let i = 0; i < unique_name_to.length; ++i) {
          writer.uint32(10);
          writer.bytes(unique_name_to[i]);
        }
      }
    }
    ...
    constructor(to: Array<Uint8Array> = []) {
      this.to = to;
    }
  }

  @unmanaged
  export class set_last_time_arguments {
    static encode(message: set_last_time_arguments, writer: Writer): void {
      if (message.value != 0) {
        writer.uint32(8);
        writer.uint64(message.value);
      }
    ...
    static decode(reader: Reader, length: i32): set_last_time_arguments {
      const end: usize = length < 0 ? reader.end : reader.ptr + length;
      const message = new set_last_time_arguments();
